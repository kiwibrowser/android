...
class MockVisitor : public spdy::SpdyFramerVisitorInterface {
 public:
  MOCK_METHOD1(OnError,
               void(http2::Http2DecoderAdapter::SpdyFramerError error));
  MOCK_METHOD3(OnDataFrameHeader,
               void(spdy::SpdyStreamId stream_id, size_t length, bool fin));
  MOCK_METHOD3(OnStreamFrameData,
               void(spdy::SpdyStreamId stream_id,
                    const char* data,
                    size_t len));
  MOCK_METHOD1(OnStreamEnd, void(spdy::SpdyStreamId stream_id));
  MOCK_METHOD2(OnStreamPadding, void(spdy::SpdyStreamId stream_id, size_t len));
  MOCK_METHOD1(
      OnHeaderFrameStart,
      spdy::SpdyHeadersHandlerInterface*(spdy::SpdyStreamId stream_id));
  MOCK_METHOD1(OnHeaderFrameEnd, void(spdy::SpdyStreamId stream_id));
  MOCK_METHOD3(OnControlFrameHeaderData,
               bool(spdy::SpdyStreamId stream_id,
  ...
  MOCK_METHOD2(OnSetting, void(spdy::SpdySettingsId id, uint32_t value));
  MOCK_METHOD0(OnSettingsAck, void());
  MOCK_METHOD0(OnSettingsEnd, void());
  MOCK_METHOD2(OnPing, void(spdy::SpdyPingId unique_id, bool is_ack));
  MOCK_METHOD2(OnGoAway,
               void(spdy::SpdyStreamId last_accepted_stream_id,
                    spdy::SpdyErrorCode error_code));
  ...
  MOCK_METHOD2(OnWindowUpdate,
               void(spdy::SpdyStreamId stream_id, int delta_window_size));
  MOCK_METHOD1(OnBlocked, void(spdy::SpdyStreamId stream_id));
  MOCK_METHOD3(OnPushPromise,
               void(spdy::SpdyStreamId stream_id,
                    spdy::SpdyStreamId promised_stream_id,
                    bool end));
  MOCK_METHOD2(OnContinuation, void(spdy::SpdyStreamId stream_id, bool end));
  ...
  MOCK_METHOD2(OnUnknownFrame,
               bool(spdy::SpdyStreamId stream_id, uint8_t frame_type));
};
...
class QuicHeadersStreamTest : public QuicTestWithParam<TestParams> {
 public:
  QuicHeadersStreamTest()
    ...
    framer_ = std::unique_ptr<spdy::SpdyFramer>(
        new spdy::SpdyFramer(spdy::SpdyFramer::ENABLE_COMPRESSION));
  ...
  void SaveToHandler(size_t size, const QuicHeaderList& header_list) {
    headers_handler_ = QuicMakeUnique<spdy::test::TestHeadersHandler>();
  ...
  void WriteHeadersAndCheckData(QuicStreamId stream_id,
    ...
    QuicSpdySessionPeer::WriteHeadersImpl(
        ...
        spdy::Spdy3PriorityToHttp2Weight(priority), 0, false, nullptr);

    // Parse the outgoing data and check that it matches was was written.
    if (is_request) {
      EXPECT_CALL(visitor_,
                  OnHeaders(stream_id, kHasPriority,
                            spdy::Spdy3PriorityToHttp2Weight(priority),
                            /*parent_stream_id=*/0,
  ...
  std::unique_ptr<spdy::test::TestHeadersHandler> headers_handler_;
  QuicString body_;
  QuicString saved_data_;
  QuicString saved_header_data_;
  QuicString saved_payloads_;
  std::unique_ptr<spdy::SpdyFramer> framer_;
  ...
  for (QuicStreamId stream_id = client_id_1_; stream_id < client_id_3_;
    ...
    for (bool fin : kFins) {
      if (perspective() == Perspective::IS_SERVER) {
        ...
        for (spdy::SpdyPriority priority = 0; priority < 7; ++priority) {
  ...
  for (QuicStreamId stream_id = client_id_1_; stream_id < client_id_3_;
    ...
    for (bool fin : {false, true}) {
      for (spdy::SpdyPriority priority = 0; priority < 7; ++priority) {
        ...
        if (perspective() == Perspective::IS_SERVER) {
          spdy::SpdyHeadersIR headers_frame(stream_id, headers_.Clone());
          headers_frame.set_fin(fin);
          headers_frame.set_has_priority(true);
          headers_frame.set_weight(spdy::Spdy3PriorityToHttp2Weight(0));
...
TEST_P(QuicHeadersStreamTest, ProcessPushPromise) {
  ...
  for (QuicStreamId stream_id = client_id_1_; stream_id < client_id_3_;
       stream_id += next_stream_id_) {
    QuicStreamId promised_stream_id = NextPromisedStreamId();
    spdy::SpdyPushPromiseIR push_promise(stream_id, promised_stream_id,
                                         headers_.Clone());
    spdy::SpdySerializedFrame frame(framer_->SerializeFrame(push_promise));
    if (perspective() == Perspective::IS_SERVER) {
...
TEST_P(QuicHeadersStreamTest, ProcessPriorityFrame) {
  ...
  for (spdy::SpdyPriority priority = 0; priority < 7; ++priority) {
    for (QuicStreamId stream_id = client_id_1_; stream_id < client_id_3_;
      ...
      spdy::SpdyPriorityIR priority_frame(stream_id, parent_stream_id, weight,
                                          true);
      spdy::SpdySerializedFrame frame(framer_->SerializeFrame(priority_frame));
...
TEST_P(QuicHeadersStreamTest, ProcessPushPromiseDisabledSetting) {
  ...
  spdy::SpdySettingsIR data;
  // Respect supported settings frames spdy::SETTINGS_ENABLE_PUSH.
  data.AddSetting(spdy::SETTINGS_ENABLE_PUSH, 0);
  spdy::SpdySerializedFrame frame(framer_->SerializeFrame(data));
  stream_frame_.data_buffer = frame.data();
  stream_frame_.data_length = frame.size();
  if (perspective() == Perspective::IS_CLIENT) {
    EXPECT_CALL(
...
TEST_P(QuicHeadersStreamTest, ProcessSpdyRstStreamFrame) {
  spdy::SpdyRstStreamIR data(2, spdy::ERROR_CODE_PROTOCOL_ERROR);
  spdy::SpdySerializedFrame frame(framer_->SerializeFrame(data));
  EXPECT_CALL(*connection_,
              CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA,
...
TEST_P(QuicHeadersStreamTest, RespectHttp2SettingsFrameSupportedFields) {
  ...
  spdy::SpdySettingsIR data;
  // Respect supported settings frames spdy::SETTINGS_HEADER_TABLE_SIZE,
  // spdy::SETTINGS_MAX_HEADER_LIST_SIZE.
  data.AddSetting(spdy::SETTINGS_HEADER_TABLE_SIZE, kTestHeaderTableSize);
...
TEST_P(QuicHeadersStreamTest, RespectHttp2SettingsFrameUnsupportedFields) {
  SetQuicReloadableFlag(quic_send_max_header_list_size, true);
  spdy::SpdySettingsIR data;
  // Does not support spdy::SETTINGS_MAX_CONCURRENT_STREAMS,
  // spdy::SETTINGS_INITIAL_WINDOW_SIZE, spdy::SETTINGS_ENABLE_PUSH and
  // spdy::SETTINGS_MAX_FRAME_SIZE.
  data.AddSetting(spdy::SETTINGS_MAX_CONCURRENT_STREAMS, 100);
  data.AddSetting(spdy::SETTINGS_INITIAL_WINDOW_SIZE, 100);
  data.AddSetting(spdy::SETTINGS_ENABLE_PUSH, 1);
  data.AddSetting(spdy::SETTINGS_MAX_FRAME_SIZE, 1250);
  spdy::SpdySerializedFrame frame(framer_->SerializeFrame(data));
  EXPECT_CALL(
      *connection_,
      CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA,
  ...
  EXPECT_CALL(
      *connection_,
      CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA,
                      QuicStrCat("Unsupported field of HTTP/2 SETTINGS frame: ",
                                 spdy::SETTINGS_INITIAL_WINDOW_SIZE),
                      _));
...
TEST_P(QuicHeadersStreamTest, ProcessSpdyGoAwayFrame) {
  spdy::SpdyGoAwayIR data(1, spdy::ERROR_CODE_PROTOCOL_ERROR, "go away");
  spdy::SpdySerializedFrame frame(framer_->SerializeFrame(data));
  EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA,
...
TEST_P(QuicHeadersStreamTest, ProcessSpdyWindowUpdateFrame) {
  spdy::SpdyWindowUpdateIR data(1, 1);
  spdy::SpdySerializedFrame frame(framer_->SerializeFrame(data));
  EXPECT_CALL(*connection_,
              CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA,
